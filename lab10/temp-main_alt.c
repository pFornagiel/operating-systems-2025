...
pthread_mutex_t mutex_patients = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_pharma = PTHREAD_MUTEX_INITIALIZER;
// doktor śpi dopóki:
//  • nie będzie 3 pacjentów i >=3 leków  OR
//  • nie będzie potrzeby uzupełnienia leków (<3 i jest farmaceuta czekający)
pthread_cond_t  doctor_cv     = PTHREAD_COND_INITIALIZER;

// pacjenci czekają, aż będzie miejsce w poczekalni (max 3)
pthread_cond_t  seat_cv       = PTHREAD_COND_INITIALIZER;
// farmaceuci czekają, aż apteczka będzie miała <MEDKIT_CAPACITY
...
void* patient(void* arg){
  ...
  int try_wait = 1;
  while(try_wait){
    pthread_mutex_lock(&mutex_patients);
    ...
    if(can_patients_come == 0){
      ...
      pthread_mutex_unlock(&mutex_patients);
      ...
      while(are_patients_waiting == 1){
        pthread_cond_wait(&seat_cv, &mutex_patients);
      }
...
void* pharmacist(void *arg)
{
  ...
  free(arg);
  pthread_mutex_lock(&mutex_pharma);
  ...
  while(medkit >= 3){
    pthread_cond_wait(&pharm_wait, &mutex_pharma);
  }
  ...
  while(is_refilling == 0) {
    ...
    pthread_cond_signal(&doctor_cv);
    pthread_cond_timedwait(&refill_cv, &mutex_pharma, &ts);
  }
  ...
  // is_refilling = 0;
  pthread_cond_signal(&doctor_cv);
  pthread_mutex_unlock(&mutex_pharma);
  ...
  while(1){
    ...
    if(medkit >= 3){
      while (! (waiting_room == 3 && medkit >= 3) ) {
            pthread_cond_wait(&doctor_cv, &mutex_patients);
      }
    } else {
      while (!  (medkit < 3 && pharmacists_waiting > 0)) {
            pthread_cond_wait(&doctor_cv, &mutex_pharma);
      }
    }
